...
that appeared in the gray-box solution, by using the gray-box solution.
...
The adjoint method can be applied to the twin model to estimate the gradient of 
any objective function with respect to $c$. 
It is explained that the adjoint gradient of the twin model can drive the optimization of 
...
Recall from Chapter \ref{chap 1} that a simulator is open-box if the condition
\begin{itemize}
...
quadrature weights and the grid-box solution.
...
The key to inferring $F$ in \eqref{eqn: 1D conceptual} is to leverage the gray-box space-time solution.
I can not prove the inferrability for the general form \eqref{eqn: 1D conceptual}.
Instead, the inferrability can be partially justified by the following theorem
