...
In practice, the minimization of $\mathcal{M}_u$ can be expensive if the twin model uses
...
which is sketched in Algorithm \ref{algo: outline heuristics}, where
$\mathcal{A}$ indicates a finite set containing integers that index the basis functions in the
basis dictionary.\\
    ...
    \begin{algorithmic}[1]
        ...
        \LOOP
            \STATE Minimize solution mismatch:
                   $\alpha \leftarrow \argmin_{\alpha} \mathcal{M}
                    \left( \tilde{F}(\mathcal{A}, \alpha), \boldsymbol{u}\right)$
...
\end{equation}
where $\mathcal{M}$ is defined in \eqref{eqn: solution mismatch},
...
$\left(\boldsymbol{j}_l, \boldsymbol{\eta}_l\right)$, 
and let 
...
$(j,\eta)$ to be a set of the sigmoid bases:
\begin{equation}
...
\STATE
       ...
       $
           \alpha_{\mathcal{A}} \leftarrow \argmin_{\alpha} \mathcal{M}_u
           \left(\sum_{i \in \mathcal{A}} \alpha_i \phi_i\right)
       $
