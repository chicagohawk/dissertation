...
basis functions,
and gives an algorithm for training the twin model. To reduce the computational cost of the
...
\begin{equation*}\begin{split}
    ...
    &f(u) \in V_j \Leftrightarrow f(u-\frac{\eta}{2^j}) \in V_{j},
                    \; j\in \mathbb{Z},\, \eta\in \left\{\eta_0, \eta_0\pm 1, \eta_0\pm 2,\cdots\right\},
\end{split}\end{equation*}
where $\eta_0\in \mathbb{R}$ is an arbitrary constant.
...
\begin{equation}
    ...
    \eta\in \left\{\eta_0, \eta_0\pm 1, \eta_0\pm 2,\cdots\right\}
...
are chosen manually, such that the flux can be well approximated.
The chosen basis are $\phi_{j, \eta}$ for $j=3$, $\eta=0,1,\cdots, 8$, which are shown in 
...
To avoid overfitting in \eqref{eqn: twin min mismatch}, the
...
The minimization problem \eqref{eqn: solution mismatch L1 norm} can be solved by a gradient-based
...
are different from the initial condition in the training solution. 
...
in training a twin model. Two criterions are needed. 
The first criterion determines ``which'' basis shall be chosen as the candidate basis 
to add to or delete from the dictionary, based on some metric of the ``significance'' of 
the basis.
The second criterion determines ``whether'' the candidate basis needs to be added or deleted, based
on whether the approximation is sufficiently accurate.
The two criterions are developed in this section. \\
...
The technique of $k$-fold cross validation, which has been discussed in Section \ref{sec: flux param},
can be applied.
...
Therefore, a small $k$ is preferrable. $k=2$ will be used in the following numerical examples.\\

%\subsection{Algorithm}
%\label{sec: twin algo}
Based upon the above developments, a twin model algorithm 
...
shown in Figure \ref{fig: outline heuristics}.
The algorithm starts from training a twin model using 
a starting basis dictionary $\boldsymbol{\phi}_{\mathcal{A}}$. 
...
10-19). The forward step firstly finds the most promising candidate 
...
in Section \ref{sec: adaptive basis} and \ref{sec: trunc error}. 
