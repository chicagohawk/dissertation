...
the cardinality of $\mathcal{A}$ can increase as the number of variables increases, and
...
If the underlying function is assumed to be smooth\footnote{
To be precise, the underlying functions are in the Sobolev space $H^2(\Omega)$
where $\Omega$ is a $d$-dimensional cube.},
one can use Smolyak's sparse grid which involves 
$\mathcal{O}\left( p(\log p)^{d-1} \right)$ bases \cite{Smolyak}.\\


In many applications, 
most bases are insignificant so their coefficients can set to zero without affecting the approximation 
performance \cite{PCE, L1 basis pursuit, match pursuit, Lasso variable selection}. 
To exploit the sparse structure, dictionary
shall be constructed adaptively based on the function evaluations.
There are several methods that can adaptively determine the insignificant bases 
based on the function evaluations, which can result
in a sparse representation, such as Lasso regularization
