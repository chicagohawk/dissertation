...
ditermines the discrete adjoint vector $\boldsymbol{\lambda}$.
...
\end{equation}
By storing the independent variables of each operation, 
the gradient of the output with respect to the input variables 
can be computed using the chain rule \cite{AD review}. For example,
to compute the derivative $\dot{y} =\frac{\partial y}{\partial x_1}$ 
\footnote{Let $\dot{z}.$ 
denote the gradient of $z$ with respect to $x_1$, for any dependent variable $z$}
in \eqref{eqn: eg y=fx}, 
one can compute the derivatives with respect to $x_1$
for every operation in \eqref{eqn: elementary operation} , 
\begin{equation}\begin{split}
    \dot{w}_1 & =1\\
    \dot{w}_2& =0\\
    \dot{w}_3 & = w_2 \dot{w}_1 + w_1 \dot{w}_2\\
    \dot{w}_4 & = \cos (w_1) \dot{w}_1\\
    \dot{y} &= \dot{w}_3 + \dot{w}_4
\end{split}
\label{eqn: elementary derivative}
\end{equation}
Because the operations in \eqref{eqn: elementary operation} are elementary, their derivatives
\eqref{eqn: elementary derivative} can be hardwired in an AD software and are straightforward
to compute. Subsequently, $\dot{y}$ can be computed by
repeatedly substituting the derivatives
\begin{equation}\begin{split}
    \dot{y} & = \dot{w}_3 + \dot{w}_4\\
            & = w_2 \dot{w}_1 + w_1 \dot{w}_2 + \cos (w_1) \dot{w}_1 \\
            & = x_2 \cdot 1 + x_1 \cdot 0 + \cos(x_1) \cdot 1\\
            & = x_2 + \cos(x_1)
\end{split}\end{equation}
See \cite{AD review} for a thorough review of AD. 
There are many free AD softwares available for various languages, such as
\emph{ADOL-C} (C, C++) \cite{adolc}, \emph{Adiff} (Matlab) \cite{adiff}, and \emph{Theano} (Python)
\cite{theano}.\\

The adjoint method has seen wide applications in many areas, including the aerodynamic design
