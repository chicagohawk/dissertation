...
%, or by running a low-fidelity simulation \cite{MFO trust region, Alexandrov trust region}.
...
Stationary Gaussian process is a type of stochastic process that is used ubiquitously in Bayesian optimization
\cite{GP book}.
...
Because a PDE simulation can be viewed as 
...
%
%Experimental results can be simulated by computer simulations. Experiments can 
%have noisy observations,
%and simulations can have unknown, tunable parameters. Model calibration is a
%methodology to calibrate the
%parameters, to quantify the uncertainties\footnote{For example, 
%the uncertainties of the observation errors and the parameters.},
%and to predict future experimental and simulational results based on
%sampled experimental and simulational results \cite{KennedyOhagan1, KennedyOhagan2,
%model calibrate field data}.
%My thesis considers calibrating a twin model so its space-time solution matches 
%the gray-box model. Therefore, by analogy, the ``experiments''
%corresponds to the gray-box simulations, whereas the ``simulations''
%corresponds to the twin-model simulations.\\
%
%
%This should be near the beginning of of this subsection, to motivate why you talk about model calibration
%You never said the purpose of this paragraph. It's very hard to read this long without knowing why you are talking about this. What are you doing with (1.15)? What is known and what is unknown? Is z_i given, or to be estimated? Is c_i given, or to be estimated? What is a calibration problem? What does pieces in (1.15) represent iin the setting of a "model calibration" problem?
%
%Experimental and their simulational results 
%are accompanied by many sources of uncertainties. In the following, I explain several 
%sources relevant to my thesis, which are initially given by Kennedy and O'Hagan \cite{KennedyOhagan2}:
%\begin{enumerate}
%    \item \emph{Parameter uncertainty} refers to the uncertainty about the parameter
%          values in a simulational model. Parameter uncertainty can be eliminated if the
%          true values are known and specified in a simulation.
%    \item \emph{Model inadequacy} relates to the fact that models are barely perfect.
%          A model may not exactly reflect the true, underlying process
%          no matter what parameters it takes.
%    \item \emph{Residual variability} accounts for the variability in repeated experiments.
%          Experiments can involve randomness, either because the input condition 
%          are not fully controllable in practice, 
%          or because the experimental process involves randomness intrinsically.
%    \item \emph{Code uncertainty} refers to the uncertainty in predicting the output
%          of a simulation code. Even if the code gives deterministic ouput for any input,
%          the output is not known until one runs the simulation with this input. 
%    \item \emph{Observation error} 
%          
%\end{enumerate}
%
%Important to note that not just calibrating parameters, but calibrating functions, flux F.
%
%
%Kennedy -> unified
%But extra cost, I use \cite{non param calibrate 1, non param calibrate 2, non param calibrate 3}
%
%  may not reproduce experimental results 
...
% without using any evaluation of the underlying function,
...
Conventionally, the dictionary for the sparse approximation needs to
...
%of the significant bases are unknown a prior.
