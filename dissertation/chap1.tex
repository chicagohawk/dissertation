...
change as the system evolves over time, such as the conservation of mass, momentum, and energy. 
...
Optimization constrained by conservation laws are present in many engineering applications.
For example, in gas turbines, the rotor blades can operate at a temperature close to 2000K 
...
to circulate coolant air whose dynamics are governed by the Navier-Stokes equation. The pressure
used to drive the coolant flow is provided by the compressor, resulting in a penalty on the 
turbine's thermo-dynamic efficiency. Engineers are thereby interested in optimizing the coolant
channel geometry in order to suppress the pressure loss. Another example is the field control of petroleum
reservoir. In petroleum reservoir, the fluid flow of various phases and chemical components 
is dictated by porous medium transport equations. The flow can be passively and actively controlled by a variety of techniques \cite{first reservoir opt},
such as the wellbore pressure control, the polymer injection, the hydrolic fracturing, and the steam heating.
...
the control towards the optimal with reduced number of simulation.
...
increases with the control's degree of freedom. The two costs are multiplicative.
The multiplicative effect compromises the impact of computational efforts among field engineers.\\

Fortunately, the cost due to iteration can be alleviated by adopting gradient-based optimization algorithms.
...
To keep track of the back propogation, the simulator source code needs to be modified.
In real-world industrial simulators, adjoint is scarcely available because most source codes are proprietary and/or legacy.
...
one where the solution remains the same at every timestep.\\
...
can be approximated up to a constant by indefinite integral. Let $\tilde{F}$ be the approximation for $F$.
...
$\tilde{u}$ is the solution associated with $\tilde{F}$.
Thereby, the adjoint method can be applied to \eqref{eqn: 1D conceptual inferred} to efficiently compute the gradient, which estimates the true gradient.
...
A simulator is defined to be \textbf{gray-box} if the following two conditions are met:
\begin{enumerate}
    \item the adjoint is unavailable, and is infeasible to implement into the source code.
...
For example, \textit{OpenFOAM} is an open-source fluid simulator where adjoint can be implemented directly
...
is named \textbf{black-box}. For example, \textit{Aspen}, an industrial chemical reactor simulator, provides neither
...
using the twin model. My thesis will only investigate gray-box simulators.\\
...
initial condition, and the source term are known. This assumption is valid for some applications,
...
equations. The initial condition is usually given at the equilibrium state, the boundary
...
Although it's public knowledge that the flux satisfies the Darcy's law, 
the mechanism that modifies the rock permeability and flow viscosity due to the injected
polymer can be unavailable. Thereby the flux is partially unknown.
...
is large. We assume that the computational cost
...
Based upon previous research, Chapter 3 develops an optimization algorithm that takes 
...
function values but no gradient information, thus is useful when the gradient
is unavailable, unreliable, or too expensive to obtain \cite{gradfreereview}. 
...
DFO methods can be categorized into local methods and global methods.
Local methods seek a local optimum which is also the global optimum for convex problems. 
An important local method is the
trust-region method. Trust-region method introduces a surrogate model that is
cheap to evaluate and presumably accurate within a trust region: an adaptive neighborhood around the current iterate. 
At each iterate, the surrogate is optimized in a domain bounded by the trust region to generate
candidte steps for additional objective evaluations.
...
Example methods include the branch-and-bound search, evolution methods, and Bayesian methods. 
...
in the course of the search. The bounds are usually obtained through the assumption of the
Lipschitz continuity or statistical bounds for the objective function. 
...
Because many real-world problems are non-convex, global methods are preferred to local methods
if the global optimum is desired. Besides, it's important to notice that some DFO methods 
can be modified to admit gradient information if available. The details are discussed in section
\ref{section: GBO}.\\
...
Similar to DFO, GBO can also be categorized into local methods and global methods.
Examples of local GBO methods include the gradient descent method, the (nonlinear) conjugate gradient 
methods, and the quasi-Newton methods. The gradient descent method and
the (nonlinear) conjugate gradient methods
choose the search step in the direction of either the gradient or a conjugate gradient
...
Notice that some local DFO methods can be modified to admit gradient information. For instance,
...
Global GBO methods search for the global optimum using gradient values.
Many global GBO methods can trace their development to corresponding DFO methods. 
...
as the branch-and-bound method. But the search in each partition is performed
by gradient-based algorithms such as BFGS. Similarly, some gradient-based evolution methods,
...
can be viewed as gradient variations of corresponding derivative-free counterparts.
My thesis is particularly interested in the gradient-based Bayesian optimization method.
...
available objective and gradient values. In addition, the search step is optimal 
under the Bayesian metric. The advantage of such properties can be justified
when the objective and gradient evaluations
are dominantly more expensive than running the optimization algorithm. Besides, the
Baysian optimization method is provably convergent even if the gradient values are estimated
inexactly. Under some assumptions of the objective and the inexact gradient, the convergence 
can be proven regardless of the accuracy of the gradient estimation. Details of the proof
are given in Section \ref{sec: chap3 convergence}.\\
...
control variables. GBO methods can be efficiently applied to 
optimization constrained by open-box simulators, because the gradient is efficiently computable by
the adjoint method.
My thesis extends GBO to optimization constrained by gray-box simulators by estimating 
...
depending on whether the linearization or the discretization is excuted first.
The continous adjoint method linearizes the continuous PDE $F(u,c) = 0$ first,
then discretizes the adjoint equation \eqref{eqn: adjoint}. The discrete adjoint method 
discretizes $F(u,c)=0$ first, then linearizes the discretized adjoint equation.
...
(e.g. $\exp$, $\sin$). By storing the operations in addition to their output values, 
...
to gray-box simulations. To break this limitation, Chapter \ref{chapter 2} develops the twin model 
...
\end{equation*}
although orthonormal bases, such as the Hermite polynomials, are preferable in most applications.
...
\end{equation}
where $\{\phi_i\}_{i\in \mathcal{A}}$ is called a basis dictionary. 
For example, in polynomial approximations, the basis dictionary usually consists of
the basis whose total polynomial degree does not exceed $p\in \mathbb{N}$.
Given a basis dictionary, the coefficients can be determined by projection \cite{PCE}
or by least squares, such that the error between approximations
and function evaluations is minimized. My thesis parameterizes the twin-model flux $\tilde{F}$ and
...
as the basis complexity (e.g. the total degree $p$ of the polynomial basis) increases.
...
performance. There are several methods that can determine the insignificant bases which result 
...
to the approximation error, where $\lambda>0$ is a tunable parameter. 
It balances the approximation error and the number of non-zero
coefficients. Matching pursuit adopts a greedy, stepwise approach to construct a basis. 
...
to the dictionary. The backward step searches over the current dictionary, and removes the 
...
Parameter uncertainty refers to uncertain parameters of a model.
...
the true parameters are known and specified in a simulation. 
Model inadequacy relates to the fact that no model is perfect.
...
is eliminated. For example, in the same petroleum reservoir simulation, model inadequacy would 
...
particular control variables and model parameters until the code is run. 
In theory, code uncertainty disappears if the model is simulated densely
over its entire input domain. In practice, however, the simulation output can only be interpolated
...
measurements. This error represents the variability in the experimental response if
the experiment were repeated multiple times under the exact same settings.\\
...
error for the $i$th observation. $\delta_i$'s are assumed to be i.i.d. Gaussian distributed,
which models the idiosyncratic observation error for individual experiments. $\epsilon(\cdot)$ accounts
...
the smoothness by tuning hyperparameters like the correlation length. To incorporate the code
uncertainty, $y(\cdot, \cdot)$ is modeled by a joint Gaussian process that distributes 
on $c$ and $\theta$.
Usually, $y(\cdot, \cdot)$ is assumed to be independent of $\epsilon(\cdot)$.
We refer to the reference \cite{markov property} for a justification of the independence assumption.
The parameter uncertainty is accounted for by assuming a prior distribution on $\theta$.
...
of a function $\xi(\cdot)$ in a bounded set $\mathcal{C}\subset \mathbb{R}^d$. However,
Bayesian optimization distinguishes from other methods by maintaining a probabilistic
model for $\xi$. The probabilistic model is exploited to 
...
the posterior is also a Gaussian process with the mean and covariance
\begin{equation}\begin{split}
    \tilde{m}(c) & 
...
various covariance functions, we refer to the Chapter 4 in \cite{GP book}.
Among such choices, the Mat$\acute{\textrm{e}}$rn $5/2$ kernel 
\begin{equation}
    K(c, c^\prime) = 
...
smoothness. My thesis will focus on using the Mat$\acute{\textrm{e}}$rn $5/2$ kernel. 
...
fully-Bayesian approach. We refer to the reference \cite{practical Bayesian} for the details 
...
investing the next sample at $c\in \mathcal{C}$. The location of the next sample is 
...
than the PI, and requires no extra tunable parameters.
Because \eqref{eqn: EI} has a closed-form gradient, the acquisition function
can be maximized by a global GBO method, e.g. StoGo, to obtain its global maximum.\\
...
constraints. The constraints can be enforced by modifying the objective, such as
...
admits gradient information \cite{derivative RKHS, grad coKriging}. Besides, such
evaluations can be inexact, possibly corrupted by model inadequacy and observation error.
These properties are important because my thesis 
will use both the exact function evaluation and the inexact twin-model gradient 
