...
\end{equation}
\eqref{eqn: elementary operation} can be represented by the graph in Figure \ref{fig: AD graph}.
\begin{figure}
    \begin{center}
    \includegraphics[width=4cm]{../AD_tree.png}
    \caption{The computational graph for \eqref{eqn: elementary operation}. The yellow nodes indicates
             the input variables, the blue node indicate the output variable, and the white
             nodes indicate the intermediate variables. Arrows indicate elementary operations.
             The begining and end nodes of each 
             arrow indicate the independent and dependent variables.}
    \label{fig: AD graph}
    \end{center}
\end{figure}
...
variable $z$ in \eqref{eqn: eg y=fx}.
To compute the derivatives $\bar{c}_1 = \frac{\partial \xi}{\partial c_1}$ and
$\bar{c}_2 = \frac{\partial \xi}{\partial c_2}$,
one can propogate the derivatives backward in the computational graph, 
\begin{equation}\begin{split}
    \bar{w}_4 & = 1\\
    \bar{w}_3& =1\\
    \bar{w}_2 & = \bar{w}_3 \frac{w_3}{w_2} = 1\cdot \cdot w_1\\
    \bar{w}_1 & = \bar{w}_4 \frac{\partial w_4}{\partial w_1} + \bar{w_3} 
                  \frac{\partial w_3}{\partial w_1} = \cos(w_1) + w_2\\
...
their derivatives can be hardwired in an AD software. 
Notice each arrow is traversed for one and only one time in the backward propogation 
\eqref{eqn: elementary derivative}. Therefore, the backward
gradient computation has a similar cost to the forward output evaluation, regardless of
