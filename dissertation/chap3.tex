...
errors separately. Instead, my thesis treats the gradient error as a whole 
as a model inadequacy. The model inadequacy is then calibrated using a Bayesian approach.\\
...
The deterministic and correlated error can be treated as model inadequacy as discussed
in Section \ref{sec: model calibration}. Let $\nabla\xi$ be the true gradient,
...
because $\nabla \xi$ and $\xi_{\tilde{\nabla}}$ are deterministic. It also 
sets the regression parameter, $\rho$ in \eqref{eqn: model calibration},
to be $1$ for computational simplicity.\\
...
\begin{figure}[htbp]\begin{center}
    \includegraphics[width=9cm]{../NS_compute_time.png}
...
Firstly, consider constant-in-time injection rates at the injectors.
Define 
\begin{equation}
...
\end{equation}
which is the negative oil residual minus the water cost at time $t$.
$\lambda = 0.4$ is the cost of water per unit volume. $I_{\texttt{inj}i}$ is the injection rate
...
the covariance matrix for evaluating the posterior is $N(d+1)$. 
...
Clearly, such practice is theoretically flawed, because the gradient error
can not be estimated or bounded, and the optimization may not converge at all.
...
\begin{figure}[htbp]\begin{center}
    \includegraphics[width=7.5cm,height=5.7cm]{../reservoir_obj_grad_descent.png}
