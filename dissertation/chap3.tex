...
%Re-Discuss "reuse".
%Add algorithm, previous iteration twin model,
%for existing basis,
%remove basis, then use the pruned dictionary as initial dictionary.
%Then algo2.
%
%In line 3 of Algorithm \ref{algo: outline heuristics},
...
%The dictionary for the twin model of the last iteration can be used as an initial guess
%of the dictionary for the current twin model.
%\begin{algorithm}[htbp]
%\begin{algorithmic}[1]
%\REQUIRE{$\mathcal{A}, \alpha_{\mathcal{A}}$ of the trained twin model at the previous iteration.}
%\STATE Compute $\overline{\mathcal{M}}$ by $k$-fold cross validation. $\overline{M}_0\leftarrow 
%       \overline{\mathcal{M}}$.
...
%    \IF{$\overline{M} < \overline{M}_0$}
%        \STATE $\overline{M}_0 \leftarrow \overline{M}$
%    \ELSE 
...
%\ENSURE $\mathcal{A}$
%\end{algorithmic}
%\caption{Prune the basis dictionary of previously trained twin model.}
